...
void PutPixel(img_buffer_t *buffer, int32_t x, int32_t y, const color_t color) {
  if (x < 0 || y < 0 || x >= (int32_t)buffer->width ||
      y >= (int32_t)buffer->height) {
    return;
  }
  buffer->pixels[(size_t)y * buffer->width + x] = color;
}

void HorizontalLine(img_buffer_t *buffer, int32_t y, int32_t x0, int32_t x1,
                        const color_t color) {
  if (y < 0 || y >= (int32_t)buffer->height) {
    return;
  }
  ...
  if (x1 >= (int32_t)buffer->width) {
    x1 = (int32_t)buffer->width - 1;
  }
  if (x0 > x1) {
    return;
  }
...
void VerticalLine(img_buffer_t *buffer, int32_t x, int32_t y0, int32_t y1,
                      const color_t color) {
  if (x < 0 || x >= (int32_t)buffer->width) {
    return;
  }
  ...
  if (y1 >= (int32_t)buffer->height) {
    y1 = (int32_t)buffer->height - 1;
  }
  if (y0 > y1) {
    return;
  }
...
static void DrawLineH(img_buffer_t *buffer, int32_t x0, int32_t x1, int32_t y0,
                      int32_t y1, const color_t color) {
  if (x0 > x1) {
    int32_t x_temp = x0;
    x0 = x1;
    x1 = x_temp;
...
static void DrawLineV(img_buffer_t *buffer, int32_t x0, int32_t x1, int32_t y0,
                      int32_t y1, const color_t color) {
  if (y0 > y1) {
    int32_t x_temp = x0;
    x0 = x1;
...
void DrawLine(img_buffer_t *buffer, int32_t x0, int32_t y0, int32_t x1,
              int32_t y1, const color_t color) {
...
void DrawRectangle(img_buffer_t *buffer, int32_t x0, int32_t y0, int32_t x1,
                   int32_t y1, bool outline, bool fill,
                   const color_t outline_color, const color_t fill_color) {
  ...
  if (fill) {
    for (int32_t y = top_y; y <= bottom_y; y++) {
      HorizontalLine(buffer, y, left_x, right_x, fill_color);
    }
  }

  if (outline) {
    HorizontalLine(buffer, top_y, left_x, right_x, outline_color);
    HorizontalLine(buffer, bottom_y, left_x, right_x, outline_color);
    VerticalLine(buffer, left_x, top_y, bottom_y, outline_color);
    VerticalLine(buffer, right_x, top_y, bottom_y, outline_color);
  }
}

void DrawCircle(img_buffer_t *buffer, int32_t cx, int32_t cy, int32_t radius,
                bool outline, bool fill, const color_t outline_color,
  ...
  while (y >= x) {
    if (fill) {
      HorizontalLine(buffer, cy + y, cx - x, cx + x, fill_color);
      HorizontalLine(buffer, cy - y, cx - x, cx + x, fill_color);
      HorizontalLine(buffer, cy + x, cx - y, cx + y, fill_color);
      HorizontalLine(buffer, cy - x, cx - y, cx + y, fill_color);
    }
...
void DrawTriangleOutline(img_buffer_t* buffer, vec2_t v0, vec2_t v1, vec2_t v2, const color_t outline_color) {
    int32_t v0x = (int32_t)v0.x;
    int32_t v0y = (int32_t)v0.y;

    int32_t v1x = (int32_t)v1.x;
    int32_t v1y = (int32_t)v1.y;

    int32_t v2x = (int32_t)v2.x;
    int32_t v2y = (int32_t)v2.y;

    DrawLine(buffer, v0x, v0y, v1x, v1y, outline_color);
    DrawLine(buffer, v0x, v0y, v2x, v2y, outline_color);
    DrawLine(buffer, v1x, v1y, v2x, v2y, outline_color);
}
